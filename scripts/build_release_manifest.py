...
def write_github_output(version str, tag str) - None
    ...
    with open(output_path, a, encoding=utf-8) as handle
        handle.write(fversion={version}n)
        handle.write(ftag={tag}n)


def main() - None
    version_info = read_version()
    run_number = os.environ.get(GITHUB_RUN_NUMBER, 0)
    repository = os.environ.get(GITHUB_REPOSITORY, XzynAstralzPhantom)
    ...
    tag = fv{base_version}-build{run_number}
    ...
    manifest = {
        ...
        rawBaseUrl fhttpsraw.githubusercontent.com{repo_owner}{repo_name}{tag},
        preserve [config, configs, cache],
        files files,
    }

    MANIFEST_FILE.write_text(json.dumps(manifest, indent=2) + n, encoding=utf-8)
